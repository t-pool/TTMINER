	...
	if (!ec)
	{
		...
		switch (m_connection.Version()) {
			...
			case WshStratumClient::WSHPROXY:
				...
				if (m_email.empty())
				{
					os << "{\"id\": 1, \"worker\":\"" << m_worker << "\", \"method\": \"mit_submitLogin\", \"params\": [\"" << user << "\"]}\n";
				}
				else
				{
					os << "{\"id\": 1, \"worker\":\"" << m_worker << "\", \"method\": \"mit_submitLogin\", \"params\": [\"" << user << "\", \"" << m_email << "\"]}\n";
				}
				break;
...
void WshStratumClient::hashrate_event_handler(const boost::system::error_code& ec)
{
	...
	string json = "{\"id\": 6, \"jsonrpc\":\"2.0\", \"method\": \"mit_submitHashrate\", \"params\": [\"" + m_rate + "\",\"0x" + this->m_submit_hashrate_id + "\"]}\n";
	std::ostream os(&m_requestBuffer);
	os << json;
...
void WshStratumClient::submitSolution(Solution solution) {
	...
	switch (m_connection.Version()) {
		...
		case WshStratumClient::WSHPROXY:
			json = "{\"id\": 4, \"worker\":\"" +
				m_worker + "\", \"method\": \"mit_submitWork\", \"params\": [\"0x" +
				nonceHex + "\",\"0x" + solution.work.header.hex() + "\",\"0x" +
				solution.mixHash.hex() + "\"]}\n";
			break;
		case WshStratumClient::WISEPLATSTRATUM:
