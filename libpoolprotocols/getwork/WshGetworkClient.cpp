	...
	while (true)
	{
		if (m_connected || m_justConnected) {
			...
			if (m_solutionToSubmit.nonce) {
				try
				{
					bool accepted = p_client->mit_submitWork("0x" + toHex(m_solutionToSubmit.nonce), "0x" + toString(m_solutionToSubmit.work.header), "0x" + toString(m_solutionToSubmit.mixHash));
					if (accepted) {
			...
			try
			{
				Json::Value v = p_client->mit_getWork();
			...
			if (!m_currentHashrateToSubmit.empty()) {
				try
				{
					p_client->mit_submitHashrate(m_currentHashrateToSubmit, "0x" + m_client_id.hex());
				}
				catch (jsonrpc::JsonRpcException)
				{
